...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const updateCursor = (type: CursorType | undefined) => {
    try {
      ...
      if (windowLabel === "dashboard" || windowLabel === "insights") {
        // For the dashboard and insights windows, always use the normal cursor
        document.documentElement.style.setProperty("--cursor-type", "default");
        return;
      }
  ...
  useEffect(() => {
    const initializeApp = async () => {
      // Register the user's configured global shortcuts on startup
      try {
        const config = getShortcutsConfig();
        await invoke("update_shortcuts", { config });
      } catch (error) {
        console.error("Failed to register shortcuts on startup:", error);
      }
    };
    // Load data
    loadData();
  ...
  useEffect(() => {
    // For the selected AI provider, check if its curl contains {{IMAGE}}
