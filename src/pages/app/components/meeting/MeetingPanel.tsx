...
import { UseMeetingTranscription, useMeetings } from "@/hooks";
...
export const MeetingPanel = ({
  ...
  const handleSelect = async (value: string) => {
    if (isLive) return; // must Stop before switching
    if (value === "live") {
      setPastMeeting(null);
      return;
    }
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      ...
      <div className="shrink-0 flex items-center justify-between gap-2 px-3 py-1.5 border-b bg-muted/30">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {loadingPast && <Loader2 className="size-3.5 animate-spin" />}
        </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          {/* Recording controls (hidden while viewing a past meeting) */}
          {!pastMeeting && (
            <>
              <select
                value={language}
